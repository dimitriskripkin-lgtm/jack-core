...
def handle_callback(callback_data, callback_id):
    import jack_callback_handler as _jcb
...
def handle(text):
    ...
    _cmd_r = _jch.handle(_rt, text, send)
    if _cmd_r is not None: return _cmd_r
        ...
        try:
            ...
            for u in updates:
                ...
                if 'voice' in msg and chat_id:
                    import jack_voice_handler as _jvh
                    _jvh.handle_voice(msg, {'api':API,'token':TOKEN,'chat_id':CHAT_ID,'send':send,'send_keyboard':send_keyboard,'pending':PENDING_EXEC,'vibrate':vibrate})
                    continue
                if text and chat_id:
                    vibrate(40)
                    reply = handle(text)
