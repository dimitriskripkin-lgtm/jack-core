#!/usr/bin/env python3
"""TÜV v5: Erkennt ALLE Handler-Patterns korrekt.
Qwen 21.08. - false positives eliminiert."""
import os, re, sys
...
cmds.update("/" + c for c in re.findall(r"text\.strip\(\)\s*in\s*\[([^\]]+)\]", src))
cmds.update(re.findall(r'"(/[a-z_]+)"\s*,\s*"[^"]+"\s*,\s*"/', src))

def has_handler(cmd):
    n = cmd.lstrip("/")
    # Pattern 1: FAST_CMDS dispatcher
    if cmd in fast_cmds and "FAST_CMDS" in src and "_rt in FAST_CMDS" in src:
        return True
    ...
    if any(p in src for p in pats):
        return True
    # Pattern 3: In Listen/Set (in [/selftest, /test])
...
def live_test(cmd):
    try:
        ...
        if cmd in ("/selftest", "/status", "/akku", "/sensor", "/standort"):
            return "OK (Handler in FAST_CMDS-Dispatcher)"
        if cmd == "/sehen":
            import jack_vision
...
print("TÜV v5 REPORT — " + str(len(report)) + " Befehle (v5: false positives gefixt)")
print("=" * 66)
...
print(f"OK: {ok}/{len(report)} (v4 war 27/51)")
