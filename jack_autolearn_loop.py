...
def _skill_cmd_allowed(cmd):
    """SSH_SKILL_WHITELIST: nur xiaomi-jack oder lokal-safe."""
    c = (cmd or "").strip()
    low = c.lower()
    for d in ("rm -rf", "mkfs", "dd if=", "drop table", "reboot", "shutdown", "jack_secrets", "id_jack", "wget ", "curl http"):
        if d in low:
            return False, "DENY:" + d
        ...
        if _now.hour == 4 and _last_pruning_date != _today:
            try:
                ...
                if _hp.worker_target() == "xiaomi":
                    ...
                    print("P4: Honor kuehl - Pruning laeuft lokal")
                    _pr = jack_memory_pruning.run()
                log(f"PRUNING: {_pr}")
